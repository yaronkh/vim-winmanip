...
if ! exists("g:winmanip_disable_key_mapping")
    ...
    nmap <Leader>1<Left> <Plug>(JumpLeft)
    nmap <Leader>1<Down> <Plug>(JumpDown)
    nmap <Leader>1<Up> <Plug>(JumpUp)

    nmap <Leader>2<Right> <Plug>(MoveBufRight)
    nmap <Leader>2<Left> <Plug>(MoveBufLeft)
    nmap <Leader>2<Down> <Plug>(MoveBufDown)
    nmap <Leader>2<Up> <Plug>(MoveBufUp)

    nmap <Leader>3<Right> <Plug>(MoveJumpBufRight)
    nmap <Leader>3<Left> <Plug>(MoveJumpBufLeft)
    nmap <Leader>3<Down> <Plug>(MoveJumpBufDown)
    nmap <Leader>3<Up> <Plug>(MoveJumpBufUp)

    nmap <Leader>4<Right> <Plug>(MoveWinToNextTab)
    nmap <Leader>4<Left> <Plug>(MoveWinToPrevTab)

    nmap <Leader>5<Right> <Plug>(CopyBufRight)
    nmap <Leader>5<Left> <Plug>(CopyBufLeft)
    nmap <Leader>5<Down> <Plug>(CopyBufDown)
    nmap <Leader>5<Up> <Plug>(CopyBufUp)

    nmap <Leader>6<Right> <Plug>(CopyJumpBufRight)
    nmap <Leader>6<Left> <Plug>(CopyJumpBufLeft)
    nmap <Leader>6<Down> <Plug>(CopyJumpBufDown)
    nmap <Leader>6<Up> <Plug>(CopyJumpBufUp)
